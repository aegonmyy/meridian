...
- **The god-key.** Hub owner = Rebalancer owner = AgentConsumer owner = AgentConsumer AGENT =
  ...
  exercise the deposit/withdraw surface, NOT the cross-chain machinery. See SECURITY below.
...
| D2 | **Aave on all three spokes** — Arb + Base + OP | Skip OP; OP spoke-only | OP-Sepolia Aave DOES exist (pool 0xb502…, USDC underlying 0x5fd8… matches the OP spoke asset exactly). All three chains get a full Aave adapter. Base gas is low (0.017 ETH) — monitor; top up from another chain if a spoke+adapter deploy runs short. |
...
to prove/disprove**, and if it fails, the fix is a token-model change (e.g. vault asset =
