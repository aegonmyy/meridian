...
`script/DeployForkSimulation.s.sol` deploys the same full stack, hub, spoke, all three adapters, Rebalancer, AgentConsumer, in one run against forked mainnet state via `CCIPLocalSimulatorFork`, so a deployment sequence can be rehearsed locally without testnet gas expenditure or CCIP latency.
