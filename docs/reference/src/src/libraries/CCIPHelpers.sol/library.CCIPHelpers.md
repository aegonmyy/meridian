# CCIPHelpers
[Git Source](https://github.com/aegonmyy/meridian/blob/14eb4367d262c366b0c0301a0aed2d6e87141729/src/libraries/CCIPHelpers.sol)

**Title:**
CCIPHelpers

Shared types and codec functions for all CCIP messages between HubVault and SpokeVaults

All cross-chain communication in Meridian uses a single encoded CcipMessage payload.
...
ABI-encodes a CcipMessage into bytes for inclusion in a CCIP message payload

Used by both hub and spoke before calling router.ccipSend().
...
|----|----|-----------|
|`_message`|`CcipMessage`|The CcipMessage struct to encode|

**Returns**

|Name|Type|Description|
|----|----|-----------|
|`<none>`|`bytes`|ABI-encoded bytes ready for use as the CCIP message data field|


### decode

ABI-decodes bytes from a CCIP message payload back into a CcipMessage struct

Used by both hub and spoke inside _ccipReceive() after receiving a message.
...
|Name|Type|Description|
|----|----|-----------|
|`_encodedMessage`|`bytes`|Raw bytes from the CCIP message data field|

**Returns**

|Name|Type|Description|
|----|----|-----------|
|`<none>`|`CcipMessage`|Decoded CcipMessage struct ready for routing and processing|


## Structs
### AdapterInstructions
Describes a single capital movement instruction for a yield adapter

Used in both deposit and rebalance contexts — field semantics vary by message type:
...
### CcipMessage
The canonical message payload encoded in the CCIP `data` field for all Meridian messages

Encoded via abi.encode and decoded via abi.decode. The same struct is used for all
...
## Enums
### MessageType
Identifies the intent of a CCIP message so the receiver can route correctly

Encoded as a uint8 inside CcipMessage. Each value maps to a handler function
