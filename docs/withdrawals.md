...
Each leg arrival constitutes one settlement attempt for the pending withdrawal, not a guaranteed settlement. `pendingLegs` decrements and settlement is attempted; settlement remains gated (see below) and may still defer after the final leg arrives, if idle solvency is insufficient at that time.
