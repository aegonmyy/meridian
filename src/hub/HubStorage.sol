...
///      The hook set below is derived from the real call graph, and differs slightly from
///      the plan's suggested starting set.
abstract contract HubStorage is ERC4626, CCIPReceiver, Ownable, Pausable {
    ...
    ///      `quotedAssets` is the previewRedeem() value at request time, for reference and recall
    ...
    ///      A spoke with timestamp 0 is treated as stale and triggers Path 2 withdrawal.
    ...
    ///      a Path 3 recall leg that never arrives. Cancelling returns escrowed shares; the
    ///      user keeps their claim on any later-arriving leg funds via ordinary idle.
    ...
    ///      rebase on accept, honest cumulative yield eventually exceeds it permanently and
    ...
    ///      left untouched, and never clamped, since clamping corrupts pricing in the other
    ...
    /// @param assets Full USDC value owed to the withdrawer on settlement (a quote, not a promise; WI-4 claim-time pricing)
    ...
    // with `override` in exactly one module. This set differs from the plan's suggested
    // starting list because several
