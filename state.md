# Meridian Protocol: findings and design decisions
...
- Yield is socialized: a high performing market benefits every shareholder, wherever their capital happens to sit
- Losses are also socialized: a spoke exploit affects every shareholder proportionally
...
Initial design had hub specifying exact adapter instructions for withdrawals. Problem: hub doesn't know per-adapter balances on spokes, only the total spoke balance. Hub can't safely build adapter-specific withdrawal instructions without risking pulling from an adapter that doesn't have enough.
...
Hard constraints are non-negotiable: AI output is rejected if they are violated